# GAMEBOX-LINEAGE-W — log

Append-only history sister of `GAMEBOX-LINEAGE-W.md`. Each entry starts with `## <ISO timestamp> — <header>` (newest on top); body = `- [x]` (done) / `- [ ]` (pending) checkbox tasks.
