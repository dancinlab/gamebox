# HEXA_UPSTREAM_PROPOSAL §6.1 — float runtime / x1000 fixed-point arithmetic

> Proposal spec doc for hexa-lang stdlib `core.math.fixed_point` module.
...
본 문서는 hexa-lang stage1 의 float runtime broken-state (§1.10 항목) 와 percentage / ratio
계산 hand-roll 우회 (Track AG x1000 fixed-point 13 helper fn, 추정 ~558 LOC 등가 surface)
를 stdlib `core.math.fixed_point` 모듈 제안으로 정형화. 본 문서는 **proposal spec** 이지
실 PR 이 아니다 — hexa-lang sibling repo 의 디렉터리 / 커밋 / 리뷰 작업은 user explicit
approval 가 떨어져야 별도 cycle 에서 진행한다.
...
불가하다는 기존 진단 (§1.10) 의 직접 결과.

13 helper fn 이 동일 패턴을 반복:
...
fn fixed_point_x1000(num: int, denom: int) -> int
    ...
    // overflow guard: num * 1000 이 i64 한계 초과 시 saturate + warn (raw_12).
...
fn fixed_point_format(value_x1000: int, decimals: int) -> str {
    ...
    // truncate / pad to `decimals` digits
...
- `proposals/PROPOSAL_FIXED_POINT.md` — 본 문서의 hexa-lang 측 사본 (formatting 정합).
- `src/std/fixed_point.hexa` — reference impl (§3 outline 의 land 형태).
- `tests/std/test_fixed_point.hexa` — §4 의 12+ test case.

**커밋 메시지 template**:
```
add core.math.fixed_point — int-backed decimal arithmetic

Track AG-driven proposal: closure_orchestrator weighted threshold
contribution 13 helpers (~558 LOC equivalent) generalized into stdlib.

- fixed_point_x1000 / fixed_point_pct / fixed_point_format / fixed_point_parse
- additive: no breaking changes
- workaround for hexa stage1 broken float arithmetic (§1.10)
- alternative full IEEE 754 runtime rejected: ~10x cost
```
...
중 `int_to_str` / 문자열 substring / `stderr_warn` (§6.5 future) 가 필요 — 본 제안과
...
C5. hexa stage1 broken float (§1.10) 은 본 제안과 평행 issue — fp runtime fix 가 land
...
C9. `core.math.fixed_point` 네이밍은 hexa-lang convention 의 `core.io.eprintln` (§6.5
    가설) 와 정합 — sibling repo 의 actual convention 은 PR 시점에 cross-check.
C10. 본 제안 land 시 closure_orchestrator 13 helper 는 **deprecate 아님** —
...
C11. own 5 status: C-hit 13 (Track AG) + B-bench 81→12 LOC measured (Track AV §8.1) =
     `c_plus_b_pr_ready_pending_user_approval`. Track AW 는 본 status 의 spec doc
