# HEXA_UPSTREAM_PROPOSAL §6.3 — PE struct serializer (`core.bin.struct_pack/unpack`)

> Proposal spec doc for hexa-lang stdlib `core.bin` 모듈 — Win32 / PE-style struct byte
> serialization (struct_pack / struct_unpack) + PE-specific helper.
...
본 문서는 hexa-lang stage1 의 struct serialization stdlib 부재 — Win32 synthetic struct
(SOCKADDR_IN / WSAEVENT / FILE_HANDLE / PROCESS_INFORMATION / SYSTEMTIME / HWND / HDC /
HKEY / IID / VARIANT 등) 가 11 PE loader (`pe_kernel32_core` / `pe_user32_core` /
`pe_gdi32_core` / `pe_advapi32_core` / `pe_ntdll_core` / `pe_ole32_core` /
`pe_oleaut32_core` / `pe_winsock_ws2_32` / `pe_wininet_https` / `pe_schannel_tls` /
`pe_msvcrt_core` / `pe_shell32_core` / `pe_comctl32_core` / `pe_dxgi_core`) 마다 raw hex
literal byte array 로 inline hand-code 되는 상황 — 을 stdlib `core.bin.struct_pack /
struct_unpack` builtin 으로 정형화하는 제안.
...
Track K (Winsock + WININET + SCHANNEL) + Track O (KERNEL32 core) + Track Q (USER32 + GDI32
+ ADVAPI32) + Track W (KERNEL32 extend + NTDLL + OLE32 + OLEAUT32) + Track AH/AM (MSVCRT +
SHELL32 + COMCTL32 + DXGI) 누적 11 신규 PE loader × 평균 10-15 synthetic struct site /
loader ≈ **150+ hit site** (Track AO retrofit cite 110+ 이후 Track AM 에서 +DXGI / shell
extend 로 추가).

각 loader 의 패턴 (예: `lib/loader/pe_winsock_ws2_32.hexa` line 254):
```hexa
...
+ `record_pack(rec)` 자동.
**Verdict**: PARTIAL — 본 제안과 직교. record syntax (§4.5.2 의 dict literal 부재 직접
...
정합).
**Verdict**: ADOPTED — 본 spec §2 의 layout string grammar 가 Python struct module 의
...
- `proposals/PROPOSAL_PE_STRUCT.md` — 본 문서의 hexa-lang 측 사본.
- `src/std/bin.hexa` — `core.bin` reference impl (§3 outline land 형태).
- `tests/std/test_struct_pack.hexa` — §4 의 12+ test case + edge.

**커밋 메시지 template**:
```
add core.bin.struct_pack / struct_unpack — Win32 / PE struct serializer

Track K/O/Q/W/AH/AM-driven proposal: 11 PE loader × ~10-15 synthetic
struct site = ~150+ hit, ~700-1100 LOC equivalent cumulative hand-roll
generalized into stdlib.

- struct_pack(layout, values) / struct_unpack(layout, raw)
- pe_section_header_synth helper (PE 40-byte IMAGE_SECTION_HEADER)
- additive: no breaking changes
- alternative per-loader hand-roll / FFI / record syntax all rejected
- own1 정합: pure hexa, no FFI
```
...
`stderr_warn` (§6.5 future) 사용 — 본 제안과 독립 dependency. UTF-16 LE encoding 은 본
...
C9. `core.bin` 네이밍은 hexa-lang convention — sibling repo 의 actual convention (예:
    `std_bin` vs `core.bin`) 은 PR 시점에 cross-check.
C10. 본 제안 land 시 11 loader 의 hand-roll helper 는 **deprecate 아님** — migration 0
